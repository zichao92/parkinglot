...
const clearLotspy = jest.spyOn(clearLot, 'clearLot');
const fareCalculationSpy = jest.spyOn(fareCalculation, 'fareCalculation');
const checkExistencSpy = jest.spyOn(checkExistence, 'checkExistence');
...
describe('exit', () => {
  it('should an object that contains Accept as message, updated currentCapMem and an assigned allocatedObj', () => {
    ...
    checkExistencSpy.mockImplementation(() => true);
    ...
    fareCalculationSpy.mockImplementation(() => ({ cost: 2, success: true }));
    const response = exit(
      mockParams,
      mockLimit,
      mockCurrentCapMem,
  ...
  it('should warn the driver about cost calculation issue', () => {
    ...
    checkExistencSpy.mockImplementation(() => true);
    fareCalculationSpy.mockImplementation(() => ({ cost: 0, success: false }));
    const response = exit(
      mockParams,
      mockLimit,
      mockCurrentCapMem,
  ...
  it('should warn the driver that the exiting vehicle isnt found', () => {
    ...
    checkExistencSpy.mockImplementation(() => false);
    fareCalculationSpy.mockImplementation(() => ({ cost: 0, success: false }));
    const response = exit(
      mockParams,
      mockLimit,
